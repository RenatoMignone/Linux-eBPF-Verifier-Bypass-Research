...
This project investigates the security boundaries of \textbf{eBPF} (extended Berkeley Packet Filter) by moving from \textbf{theoretical vulnerability identification} to \textbf{practical exploitation}. The fundamental question is: \textit{Do logic bugs in the eBPF verifier allow memory corruption primitives, and if so, with what scope and real-world impact?} This includes distinguishing between verifier-level gaps (e.g., taint tracking loss through unprototyped calls, blocked by secondary checks) and compiler/language-level gaps (e.g., lack of struct-field-level bounds in packet memory), as well as runtime triggers that are observable but not exploitable.

\vspace{0.3cm}
\noindent
A previous research team conducted a comprehensive, rule-based assessment of the XDP SynProxy program using ISO-IEC TS 17961-2013~\cite{ISO17961} as the vulnerability catalog. They identified approximately 60 potential violations and classified roughly 9 as theoretically exploitable by analyzing the verifier's behavior under code patches. Their work established the foundation for this study but left a critical gap: theoretical analysis alone is insufficient to understand real-world impact. This work bridges that gap by implementing functional proof-of-concept exploits, collecting multi-stage evidence, and determining which verifier bypasses represent genuine memory corruption primitives versus harmless runtime behaviors.
...
5.39 & Unprototyped Function Pointer & Verifier bypass (OOB blocked) \\
5.46b & Tainted Length (Packet Buffer) & Intra-packet write \\
\hline
\end{tabular}
...
\begin{itemize}
  ...
  \item Document evidence chains showing preservation through compilation, verification, and runtime execution
...
\begin{description}
  ...
  \item[Chapter 12] Synthesizes results, discusses limitations, and provides recommendations and conclusions
\end{description}
