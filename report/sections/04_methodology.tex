...
The core methodology is the \textbf{evidence chain}: demonstrating that a vulnerability exists, passes the verifier, and is triggered at runtime. A complete evidence chain requires validation at each compilation and execution stage, creating an unbroken lineage from source code through kernel execution that proves the vulnerability's existence and exploitability.

\subsection{Stages of the Evidence Chain}

\begin{enumerate}[label=\arabic*.]
  \item \textbf{Patch Injection}: Inject a specific ISO-IEC TS 17961-2013 rule violation into \texttt{xdp\_synproxy\_kern.c} via Git patch file to ensure reproducibility and version control.
  ...
  \item \textbf{JIT Inspection} (optional): If the architecture supports readable JIT dumps, extract native instructions using \texttt{bpftool prog dump jited}. Confirm that the vulnerability is preserved through JIT compilation.
\end{enumerate}

\section{Vulnerability Injection via Git Patches}

Rather than modifying the source code directly, vulnerabilities are injected via Git patches (in the \texttt{.patch} format used by \texttt{git apply} and \texttt{git am}). This approach provides multiple technical and methodological advantages. First, patches ensure complete reproducibility—anyone can recreate the exact vulnerable state by applying the same patch to the same source file. Second, each vulnerability remains independent; applying one patch does not contaminate or interfere with others. Third, patches are easily reversible; testing tools can iterate through them sequentially, applying and removing them without manually editing code. Fourth, patches are under version control and reviewable, allowing others to verify that the vulnerability injection is minimal and targeted. Finally, automated testing tools like xvtlas can programmatically iterate through a directory of patches, automatically applying each, compiling, testing, and recording results.
...
\caption{Patch Lifecycle: From vulnerability injection through evidence chain validation}
\end{figure}
...
The compilation pipeline begins with clang, which translates C code to eBPF bytecode suitable for kernel loading. Directly after compilation, we use llvm-objdump to inspect the compiled bytecode and confirm that vulnerable code paths are present and not optimized away by the compiler. A Makefile automates compilation steps and cleanup, ensuring consistent build environment and easy reproducibility.

\subsection{Kernel Tools}

Kernel interaction is mediated through several essential tools. The bpftool utility loads programs into the kernel, captures verifier output (including log files when requested), and dumps bytecode in multiple representations (xlated, jited). The trace\_pipe interface provides access to kernel trace output, particularly \texttt{bpf\_printk} debug messages that PoC code can use to communicate status to user-space. The ip link command attaches and detaches XDP programs to specific network interfaces, a prerequisite for triggering execution.
...
All testing occurs in a KVM/QEMU virtual machine running Linux LTS 6.8, isolated from the host system. A tmux session manages multiple terminal panes simultaneously: one for kernel build and compilation, one for verifier interaction, one for trace output observation, and one for PoC execution. Simple tools like netcat assist in TCP-level testing when needed.

\section{Classification of Vulnerability Exploitability}

After executing the evidence chain, each vulnerability is classified into one of three categories, reflecting both the verifier's acceptance and the exploitation potential at runtime.
...
All findings are designed for complete independent verification. Patches are committed to version control with clear metadata describing the vulnerability injection. Compilation commands are recorded in Makefiles and can be executed identically on any machine with the same toolchain. Verifier logs, bytecode dumps, xlated bytecode, and artifact evidence are saved in structured directories under \texttt{src/exploits/<vulnerability-id>/logs/} for permanent reference and reproducibility. PoC scripts are documented with explicit trigger instructions and expected behavior.

To independently verify any finding, a reviewer needs only to check out the same kernel version (LTS 6.8), apply the same patch to the source (located in \texttt{XDPs/xdp\_synproxy/patches/}), run the same compilation commands, attempt to load the program using identical \texttt{bpftool} invocations, examine the saved artifact logs, and execute the same PoC code while observing runtime behavior. This end-to-end reproducibility is central to our methodology's credibility. All evidence artifacts are committed to the repository alongside the report.

\section{Infrastructure Modifications to Inherited Codebase}

The previous team's infrastructure provided a solid foundation but required significant modifications and enhancements to support practical exploitation with external-perspective attacks. This section documents all modifications made to their work to establish a functional testing environment.
...
\item \textbf{Solution}: Created a comprehensive cloud-init configuration with: eBPF development tools (clang, llvm, bpftool), network utilities for testing (tcpdump, netcat, tshark, hping3), development tools (tmux, vim, git, make), post-boot initialization (eBPF JIT enablement, kernel module loading), and SSH key-based authentication.
...
\item \textbf{Problem}: Manual setup of the eBPF compilation environment was required before each test, including extracting \texttt{vmlinux.h} from kernel BTF data, loading kernel modules, enabling BPF JIT, and creating necessary directories.
