...
\subsection{JIT Disassembly Unavailable on Some Platforms}

The optional JIT inspection step requires x86-64 architecture (ARM64 and RISC-V support exists but may differ), readable JIT output enabled in kernel config, and access to \texttt{/proc/sys/net/core/bpf\_jit\_kallsyms}.Without JIT output, verification can only proceed through bytecode and xlated dumps. The vulnerability is still confirmed, but the full execution path is not visible.

\textbf{Mitigation}: Xlated output is sufficient to prove the vulnerability survives verification and reaches runtime.
...
However, an end-to-end exploit to privilege escalation was not demonstrated in this report. The limitation is:
\begin{itemize}
  \item Exploitation requires additional context-specific knowledge (stack layout, ROP gadgets)
  \item Some assumptions (e.g., return address location) were not exhaustively verified
\end{itemize}

\textbf{Confidence level}: Very high that the vulnerabilities exist and are memory-corrupting; high (but not absolute certainty) that full privilege escalation is achievable with additional work.
