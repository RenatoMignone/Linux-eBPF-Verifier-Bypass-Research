...
5.39 & Taint + Range & Verifier Bypass (Runtime OOB Blocked) & Very High \\
5.46b & Taint + Bounds & Intra-Packet Corruption (Compiler/Language Gap) & Very High \\
\hline
\end{tabular}
...
\noindent
Second, the verifier \textbf{fails to properly track tainted values} (user-controlled data from packet fields) through control flow and into array indices, enabling \textbf{out-of-bounds memory access} on the eBPF stack (5.39). For 5.46b, the vulnerability is better understood as a \textbf{compiler/language-level gap}: at the eBPF bytecode level, packet accesses are unstructured pointer arithmetic, and the C-level struct field boundaries (e.g., \texttt{h\_dest[6]}) are lost during compilation. Extended testing confirmed that the verifier \textbf{correctly enforces packet-level bounds} (\texttt{data\_end}), but cannot enforce field-level bounds that do not exist in the bytecode.
...
\noindent
Vulnerability 5.39 (\textbf{verifier bypass via unprototyped function pointer}) and 5.46b (\textbf{intra-packet buffer corruption}) are not theoretical verifier gaps. They are confirmed to pass the verifier with tainted data flows intact. For 5.39, extended testing with \texttt{volatile} revealed that while the taint tracking bypass is real, a second independent verifier check (\texttt{fp + register} arithmetic) blocks the actual OOB stack write. The original code loads only because LLVM eliminates the store as dead code. For 5.46b, variant testing confirmed that intra-packet corruption occurs but cannot escape the packet buffer boundary.
...
\noindent
This project builds directly on the prior team's theoretical assessment. Where they asked \textit{``Does the verifier accept this rule violation?''}, this work asked \textit{``Can we actually exploit it?''} The prior team's classifications (exploitable, limited, safe) were largely confirmed by this practical work. This work extended their analysis by providing functional PoCs and runtime evidence, quantifying memory corruption scope and limitations, establishing a reproducible test infrastructure, documenting exploitation scope limitations: 5.39 demonstrates a confirmed verifier bypass (taint tracking loss through unprototyped calls) but the actual OOB write is independently blocked by a second verifier check; 5.46b demonstrates intra-packet corruption that cannot escape the packet buffer boundary, and distinguishing between verifier gaps (ABI compliance, taint tracking) and compiler/language gaps (struct-field-level bounds in packet memory).
...
\begin{enumerate}[label=\arabic*.]
  \item \textbf{Backport Analysis}: Test vulnerabilities against older kernel versions (5.x, 4.x) to determine historical scope
  \item \textbf{Extended Vulnerability Catalog}: Analyze additional patches from the prior team's 60+ vulnerability catalog
  \item \textbf{Automated PoC Generation}: Develop tooling to automatically generate exploit packets from verifier log patterns
  \item \textbf{Formal Verification}: Apply formal methods to verify proposed verifier patches~\cite{nelson2020bpf,vishwanathan2023bpf}
  \item \textbf{Cross-Architecture Testing}: Extend testing to ARM64 and RISC-V architectures
  \item \textbf{Fuzzing Integration}: Integrate findings with eBPF fuzzing frameworks (e.g., Syzkaller)
\end{enumerate}
