...
\begin{enumerate}[label=\arabic*.]
  ...
  \item \textbf{Clear distinction}: Between runtime triggers (ABI mismatches) and true memory corruption primitives
\end{enumerate}
...
\begin{itemize}
  \item \textbf{ABI compliance} for function pointers (all architectures)
...
Vulnerabilities 5.39 (\textbf{stack OOB write}) and 5.46b (\textbf{stack buffer overflow}) are not theoretical—they are:
\begin{itemize}
  ...
  \item \textbf{Proven memory write primitives}; full exploitation would require kernel-specific adaptation
\end{itemize}
...
\hline
5.46b & Buffer Overflow & Kernel memory write primitive (out-of-bounds write) & HIGH \\
5.39 & Stack OOB Write & Kernel memory write primitive (stack corruption) & HIGH \\
...
\begin{itemize}
  ...
  \item Quantification of memory corruption (bytes written, offsets)
  \item A reproducible test infrastructure
  \item Clear path to privilege escalation
\end{itemize}
...
\begin{enumerate}[label=\arabic*.]
  \item \textbf{Review and apply} the proposed verifier fixes (ABI checking, taint propagation)
...
\begin{enumerate}[label=\arabic*.]
  \item \textbf{Complete the exploitation}: Demonstrate full privilege escalation for 5.39 and 5.46b through ROP chain development and return address targeting
  ...
  \item \textbf{ABI chaining}: Investigate whether 5.06a/b/d can leak kernel memory or enable multi-stage attacks
\end{enumerate}
...
\begin{enumerate}[label=\arabic*.]
  \item \textbf{ABI enforcement}: Track function pointer types, validate call sites against declarations, reject mismatched argument counts and types
